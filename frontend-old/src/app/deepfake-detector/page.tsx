  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-12">
      <div className="mb-8">
        ...
        <p className="text-slate-400 text-sm mt-1">Screen an image with a trained deepfake classifier and supporting forensic diagnostics.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div onDragOver={(e) => e.preventDefault()} onDrop={(e) => { e.preventDefault(); const dropped = e.dataTransfer.files?.[0]; if (dropped) { setFile(dropped); setPreview(URL.createObjectURL(dropped)); setResult(null); } }} className="border-2 border-dashed border-slate-800 hover:border-slate-700 bg-slate-900/40 rounded-2xl p-8 flex flex-col items-center justify-center text-center relative overflow-hidden min-h-[300px]">
          {preview ? (
            ...
            <label className="cursor-pointer flex flex-col items-center gap-3">
              ...
              <span className="text-slate-500 text-xs">JPG, PNG, WEBP • Max 15MB • drag & drop supported</span>
        ...
        <div className="flex flex-col justify-center gap-4 p-6 bg-slate-900/60 border border-slate-800 rounded-2xl">
          <h3 className="text-lg font-bold text-white">Image Diagnostic Inspection</h3>
          <p className="text-slate-400 text-sm">
            A trained image classifier estimates whether the image looks synthetic. Supporting diagnostics are shown separately and are not used as proof.
          </p>
          <button
          ...
          >
            ...
            {loading ? 'Running AI model...' : 'Scan Image Now'}
          </button>
        </div>
      </div>
      ...
      {result && (
        <div className="mt-12 bg-slate-900/80 border border-slate-800 rounded-2xl p-8 flex flex-col gap-8">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6 border-b border-slate-800 pb-8">
            <div className="flex items-center gap-6">
              ...
              <div>
                ...
                <p className="text-sm text-slate-400 mt-2">Model separation confidence: <span className="text-white font-medium">{result.confidence}%</span></p>
              </div>
            </div>
            ...
            <div className="rounded-xl border border-slate-800 bg-slate-950/70 p-5">
              <div className="flex items-center justify-between gap-4 mb-4">
                ...
                <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${
                  ...
                  {result.verification.mode === 'cross_checked' ? (result.verification.agreement === 'agree' ? 'Models agree' : 'Models disagree') : 'Local model only'}
                </span>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="p-4 bg-slate-900 border border-slate-800 rounded-xl">
                  <p className="text-xs uppercase tracking-wider text-slate-500">Local ML model</p>
                  <p className="text-sm font-semibold text-white mt-1">{result.prediction}</p>
                  <p className="text-xs text-slate-500 mt-1">Independent trained classifier</p>
                </div>
                <div className="p-4 bg-slate-900 border border-slate-800 rounded-xl">
              ...
              {result.verification.external?.status === 'not_configured' && (
                <p className="text-xs text-slate-500 mt-4">Add a Reality Defender API key to the backend .env file to enable independent cross-checking.</p>
              )}
            </div>
          )}
          ...
          <div className="rounded-xl border border-slate-800 bg-slate-950/70 p-4 text-xs text-slate-500">
            <strong className="text-slate-300">Important:</strong> no image detector is perfect. This result is a screening prediction from a trained model and should not be treated as proof of authenticity or manipulation.
          </div>
        </div>
      )}
    </div>
